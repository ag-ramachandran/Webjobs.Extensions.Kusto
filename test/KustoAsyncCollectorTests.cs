...
namespace Microsoft.Azure.WebJobs.Extensions.Kusto.Tests
{
    ...
    {
        private static readonly Mock<ILogger> logger = new Mock<ILogger>(MockBehavior.Strict);
        [Fact]
        public async Task AddAsyncAccumulatesDataAsync()
        {
            ...
            var collector = new KustoAsyncCollector<Item>(logger.Object, context);
            // when
        ...
        public async Task FlushAsyncIngestsDataAsync()
        {
            ...
            KustoContext context = KustoTestHelper.CreateContext(mockIngestionClient.Object);
            var collector = new KustoAsyncCollector<Item>(logger.Object, context);
            IEnumerable<int> series = Enumerable.Range(1, 5);
            var expectedItems = new List<Item>();
            var taskResult = Task.WhenAll(series.Select(i =>
            {
